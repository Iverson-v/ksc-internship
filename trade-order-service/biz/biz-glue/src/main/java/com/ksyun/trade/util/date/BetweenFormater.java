...
public class BetweenFormater {
    ...
    public BetweenFormater(long betweenMs) {
        this(betweenMs, Level.MILLSECOND, 0);
    }
    ...
    public String simpleFormat() {
        DateUnit dateUnit = DateUnit.MILLSECOND;
        for (DateUnit unit : Arrays.asList(DateUnit.DAY, DateUnit.HOUR, DateUnit.MINUTE, DateUnit.DAY, DateUnit.SECOND, DateUnit.MILLSECOND)) {
            if (betweenMs >= unit.getMillis()) {
                dateUnit = unit;
                break;
            }
        }
        Double value = 1.0 * betweenMs / dateUnit.getMillis();
        return BigDecimal.valueOf(value).setScale(2, 4) + "" + Level.valueOf(dateUnit.name()).getName();
    }
